...
class SoldSummaryTestCase(unittest.TestCase):
    ...
    def test_sold_detail_reconciles_item_subtotal_shipping_and_total(self):
        ...
        sale_input = SaleInput(
            ...
            holo=None,
            ex=None,
            shipping={"shippingPrice": 3.50},
            payments=[],
        )

        with (
            self.app.app_context(),
            patch.dict(os.environ, {"KEY": base64.b64encode(b"0" * 16).decode()}),
        ):
        ...
        self.assertEqual(sum(item["invoice_sell_price"] for item in detail["cards"]), 20.67)
        self.assertEqual(detail["sale"]["total_amount"] - detail["sale"]["shipping_info"], 20.67)
