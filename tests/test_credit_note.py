...
class CreditNoteTestCase(unittest.TestCase):
    ...
    def test_pdf_failure_rolls_back_transaction(self, _mock):
        ...
        with self.app.app_context():
            db = get_db()
            si_before = db.execute(
                "SELECT COUNT(*) FROM sale_items WHERE sale_id = 500"
            ).fetchone()[0]
        ...
        with self.app.app_context():
            ...
            self.assertEqual(
                corrections, 0, "sales_correction row should not exist after PDF failure"
            )

            si_after = db.execute("SELECT COUNT(*) FROM sale_items WHERE sale_id = 500").fetchone()[
                0
            ]
            ...
            self.assertEqual(
                sale["total_amount"], 150.0, "total_amount should not be decremented after rollback"
            )
    ...
    def test_happy_path_commits_correction_and_returns_pdf(self, mock_gen):
        ...
        self.assertEqual(
            resp.headers["Content-Disposition"], "attachment; filename=Dobropis_test.pdf"
        )
    ...
    def test_order_return_failure_rolls_back(self):
        ...
        with self.app.app_context():
            ...
            self.assertEqual(
                si_after, 1, "sale_items should not be deleted when _orderReturn fails"
            )
