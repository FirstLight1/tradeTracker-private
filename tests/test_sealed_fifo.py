...
class SealedFIFOTestCase(unittest.TestCase):
    ...
    def _setup_test_data(self):
        ...
        db.execute(
            "INSERT INTO sealed "
            "(id, name, normalized_name, language, quantity, price, market_value, date, auction_id, cardMarketID) "
            'VALUES (10, ?, ?, "en", 5, 80.0, 100.0, "2026-01-01", 2, "900010")',
    ...
    def test_partial_sale_splits_row(self):
        ...
        with self.app.app_context():
            ...
            sold = db.execute(
                "SELECT language, quantity, price, market_value, auction_id, date, cardMarketID "
                "FROM sealed WHERE sale_id = 999"
            ).fetchall()
            self.assertEqual(len(sold), 1)
            self.assertEqual(sold[0]["language"], "en")
    ...
    def test_open_sealed_in_auction(self):
        ...
        with self.app.app_context():
            ...
            card = db.execute(
                "SELECT card_name, language, card_price, market_value, auction_id, cardMarketID "
                "FROM cards WHERE auction_id = 2"
            ).fetchone()
            ...
            sealed_content = db.execute(
                'SELECT language, cardMarketID FROM sealed WHERE name = "Mini Tin"'
            ).fetchone()
            self.assertEqual(sealed_content["language"], "en")
