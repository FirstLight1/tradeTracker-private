...
def generate_invoice(
    ...
    try:
        nameAndSurname = " ".join(
            [part.capitalize() for part in reciever.get("nameAndSurname").split(" ")]
        )
    except:
    ...
    client = Client(
        ...
        address=reciever.get("address").capitalize()
        if reciever.get("address") is not None
        else " ",
    ...
    if payment_methods and len(payment_methods) > 0:
        ...
        payment_strings = ", ".join(
            f"{item['type']} :{item['amount']}€ " for item in unique_payment
        )
    ...
    safe_name = (
        re.sub(r"[^\w.-]+", "_", reciever.get("nameAndSurname", "client") or "client").strip("_")
        or "client"
    )
    if type == "invoice":
        ...
        output_filename = (
            f"{invoice_num}_Tarchopis_c_{dn_num} _{invoice_date.strftime('%Y%m%d')}_{safe_name}.pdf"
        )
