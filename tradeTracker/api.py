...
def cardMarketTable():
    if request.method == "POST":
        ...
        languages = [str(item.get("language", "en")).strip().lower() for item in [*cards, *sealed]]
        ...
        try:
            ...
            db.executemany(
                "INSERT INTO sealed (name, normalized_name, language, quantity, price, market_value, date, auction_id, cardMarketID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                sealedToInsert,
            )
    ...
    if cards:
        try:
            for card in cards:
                ...
                rows = db.execute(
                    ...
                    (
                        card["name"].lower(),
                        card["num"].lower(),
                        card["condition"].upper(),
                        card.get("language", "en").lower(),
                    ),
                ).fetchmany(count)
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": "Failed to match cards to card ids, Error code: Ax16",
                }
            ), 400
    ...
    if sealed:
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": "There was an error while getting sealed ids, Error code: Ax17",
                }
            )
