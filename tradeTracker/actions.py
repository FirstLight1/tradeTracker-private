...
def loadExpansions():
    ...
    if getattr(sys, "frozen", False):
        ...
        expansions_path = os.path.join(
            os.path.dirname(__file__), r"data\expansions\setAbbs.json"
        )
...
def loadSealed():
    ...
    sealed_products = db.execute(
        "SELECT 's' || id as sid, name, price, market_value, date FROM sealed WHERE sale_id is NULL AND auction_id is NULL"
    ).fetchall()
    return jsonify(
        {"status": "success", "data": [dict(product) for product in sealed_products]}
    )
...
def addSealed():
    ...
    for sealed in data:
        ...
        date = (
            ...
            if sealed.get("dateAdded") is not None
            else datetime.datetime.now(datetime.timezone.utc).isoformat() + "Z"
        )
        db.execute(
            "INSERT INTO sealed(name, price, market_value, date) VALUES (?, ?, ?, ?)",
            (sealed.get("name"), price, marketValue, date),
        )
    db.commit()
...
def loadAllCards(auction_id):
    db = get_db()
    cards = db.execute(
        "SELECT * FROM cards WHERE auction_id = ?", (auction_id,)
    ).fetchall()
    return jsonify([dict(card) for card in cards]), 200
...
def invertoryValue():
    ...
    sealedValue = cur.execute(
        "SELECT SUM(market_value) FROM sealed WHERE sale_id IS NULL"
    ).fetchone()[0]
    value = (
...
def update(card_id):
    ...
    if field == "sold" or field == "sold_cm":
        db.execute(
            f"UPDATE sale_items SET {field} = ? WHERE card_id = ?", (value, card_id)
        )
        db.commit()
        return jsonify({"status": "success"}), 200
...
def linkAuctionToSale(auction_id):
    ...
    db.execute(
        "INSERT INTO barter(auction_id, sale_id) VALUES (?,?)",
        (auction_id, id["sale_id"]),
    )
    db.commit()
...
def generate_credit_note(saleId):
    ...
    # Load sealed items
    sealed_rows = db.execute(
        "SELECT * FROM sealed WHERE sale_id = ?", (saleId,)
    ).fetchall()
    sealed = [
        {
            "sealedName": r["name"],
    ...
    bulk_rows = db.execute(
        "SELECT * FROM bulk_sales WHERE sale_id = ?", (saleId,)
    ).fetchall()
    bulk = None
    holo = None
    ex = None
...
def generateSoldReport():
    ...
    sealed = db.execute(
        'SELECT se.name, se.price, se.market_value, se.auction_id FROM sealed se JOIN sales s ON se.sale_id = s.id WHERE strftime("%Y", s.sale_date) = ? AND strftime("%m", s.sale_date) = ? ',
        (year, month),
    ).fetchall()
    ...
    for s in shipping:
        temp = dict(s)
        shipping_list.append(temp["shipping_info"]) if temp[
            "shipping_info"
        ] is not None else 0
    ...
    for item_type in bulkHolo:
        ...
        bulkAndHoloList[i].update(
            {"buy_price": get_bulk_item_unit_price(item_type["item_type"])}
        )
        i += 1
...
def generatePDF(month, year, cards, sealed, bulkAndHoloList, shipping):
    ...
    pdf.add_font(
        font_family, "B", os.path.join(font_dir, "DejaVuSans-Bold.ttf"), uni=True
    )
    ...
    pdf.cell(
        0,
        10,
        f"Total Cards Sold: {len(cards) + sum(item['quantity'] for item in bulkAndHoloList) + len(sealed)}",
        0,
        1,
    )
    pdf.ln(5)
    ...
    total_sell_price = (
        ...
        + sum(item["market_value"] or 0 for item in sealed)
    ...
    for card in cards:
        ...
        chars_per_line = 25
        estimated_lines = max(
            1, (len(card_name) + chars_per_line - 1) // chars_per_line
        )
    ...
    for item in sealed:
        ...
        card_profit = (
            f"{(item['market_value'] - item['price']):.2f}€"
            if item["market_value"] and item["price"]
            else "N/A"
        )
    ...
    for row in rows:
        ...
        if row["payment_method"] != None:
            ...
            bought["Payment type"].append(
                ", ".join(payment["type"] for payment in payments)
            )
            bought["Amount"].append(
                ", ".join(str(payment["amount"]) for payment in payments)
            )
        else:
            bought["Payment type"].append("")
            bought["Amount"].append("")
...
def updatePaymentMethod(auction_id):
    ...
    db.execute(
        "UPDATE auctions SET payment_method = ? WHERE id = ?",
        (payment_method_json, auction_id),
    )
    db.commit()
...
@bp.route(
    "/recalculateCardPrices/<int:auction_id>/<string:new_auction_price>",
    methods=("GET",),
)
def recalculateCardPrices(auction_id, new_auction_price):
    ...
    sealed_items = db.execute(
        "SELECT s.id, s.market_value, s.sale_id FROM sealed s WHERE s.auction_id = ?",
        (auction_id,),
    ).fetchall()
    ...
    total_market_value = sum(card["market_value"] or 0 for card in cards) + sum(
        item["market_value"] or 0 for item in sealed_items
    )
...
def cardMarketTable():
    if request.method == "POST":
        ...
        auction["buy"] = sum(
            (float(card.get("marketValue", 0)) * 0.8) for card in cards
        )
        ...
        try:
            ...
            for card in cards:
                ...
                for _ in range(count):
                    ...
                    marketValue = (
                        float(marketValue) if marketValue is not None else None
                    )
            ...
            for item in sealed:
                ...
                for _ in range(count):
                    ...
                    sealedToInsert.append(
                        (item.get("name", None), buyPrice, marketValue, auction_id)
                    )

            db.executemany(
                "INSERT INTO sealed (name, price, market_value, auction_id) VALUES (?, ?, ?, ?)",
                sealedToInsert,
            )
...
def cardMarketOrder():
    ...
    try:
        for card in cards:
            try:
                ...
                count = 1
            rows = db.execute(
                "SELECT c.id FROM cards c LEFT JOIN sale_items si ON c.id = si.card_id WHERE c.card_name = ? AND c.card_num = ? and c.condition = ? AND si.sale_id IS NULL",
                (card["name"], card["num"], card["condition"]),
            ).fetchmany(count)
    ...
    try:
        for item in sealed:
            ...
            rows = db.execute(
                "SELECT id FROM WHERE name = ? AND sale_id IS NULL", (item["name"],)
            ).fetchmany(count)
            ids = [row[0] for row in rows]
            if len(ids) > 0:
                item["id"] = ids
...
def updateOneCard(db, name, num, condition, sellPrice):
    ...
    if cardId:
        ...
        invoice_number = (
            f"CSV-{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}-{cardId['id']}"
        )
        db.execute(
...
def search():
    if request.method == "POST":
        ...
        all_matches = [dict(m) for m in card_matches] + [
            dict(m) for m in sealed_matches
        ]
