...
class GradingService:
    ...
    def _result_matches(row, result: dict[str, Any]) -> bool:
        stored = {
            ...
            'grade_numeric': (
                Decimal(str(row['grade_numeric']))
                if row['grade_numeric'] is not None
                else None
            ),
    ...
    def create_submission(self, submission: models.GradingSubmission) -> str | None:
        ...
        try:
            ...
            cursor = self.db.execute(
                ...
                'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?)',
                (
                    grader, service_level, status.value,
            ...
            for card, allocated in zip(normalized_cards, allocations):
                ...
                landed = rounded_money(_decimal(card_price) + total) if card_price is not None else None
                self.db.execute(
                    ...
                    'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)',
                    (
                        submission_id, card['card_id'], card['grader'],
                        _db_number(card['grading_fee']), _db_number(card['prep_fee']),
                        _db_number(card['submitted_value']), _db_number(allocated),
                        _db_number(card['upcharge']), _db_number(total), _db_number(landed),
                    ),
                )
            self.db.commit()
            return None
    ...
    def complete_submission(
        ...
        try:
            ...
            if status == models.GradeStatus.GRADED:
                if all(self._result_matches(rows_by_id[result['card_id']], result) for result in results):
                    self.db.commit()
                    return None
            ...
            certificates = [
                ...
                for result in results if result['cert_number']
            ]
            ...
            if len(canonical_certificates) != len(set(canonical_certificates)):
                raise ValidationError({'cert_number': 'Certificate numbers must be unique for each grader'})
            ...
            for row, allocated in zip(rows, allocations):
                ...
                self.db.execute(
                    ...
                    (
                        result['grader'], _db_number(result['grade_numeric']), result['grade_label'],
                        result['qualifier'], result['cert_number'],
                        _db_number(result['post_grade_market_value']), _db_number(allocated),
                        _db_number(total), _db_number(landed), row['id'],
                    ),
                )
    ...
    def update_submission_status(
        ...
        try:
            ...
            if target == models.GradeStatus.RETURNED:
                ...
                submitted_at = normalize_date(current['submitted_at'], 'submitted_at', required=True)
    ...
    def grade_card(self, card_id: int, grade: models.GradingCompleteItems) -> str | None:
        ...
        try:
            ...
            self.db.execute(
                ...
                (
                    card_id, result['grader'], _db_number(result['grade_numeric']),
                    result['grade_label'], result['qualifier'], result['cert_number'],
