...
class ReportService:
    ...
    def _calculate_header(
        self, items: list[dict[str, Any]], bulk: list[dict[str, Any]]
    ) -> dict[str, Any]:
        card_count, sealed_count = (
            sum(item.get("item_type") == t for item in items) for t in ("card", "sealed")
        )

        total_buy_price = sum(
            item.get("buy_price", 0) * item.get("quantity", 1) for item in items
        ) + sum(
            CONSTANTS.BULK_ITEM_UNIT_PRICES.get(row.get("item_type", 0), "0")
            * row.get("quantity", 1)
            for row in bulk
        )

        total_sell_price = sum(
            item.get("sell_price", 0) * item.get("quantity", 1) for item in items
        ...
        for item in items:
            if item.get("item_type") == "sealed" and item["auction_id"] is not None:
                curr_margin = Decimal(item["sell_price"] * item["qty"]) - Decimal(
    ...
    def _get_buy_data_month(self, month: int, year: int) -> pd.DataFrame:
        rows = self.db.execute(
            "SELECT auction_name,auction_price, date_created, payment_method FROM auctions "
    ...
    def get_auction_data(self, id: int) -> dict[str, Any]:
        auctionInfo = self.db.execute(
            "SELECT id, auction_name, date_created FROM auctions WHERE id = ?", (id,)
        ).fetchone()
        auctionName = (
...
WHERE auction_id = ?
        ...
        bulk = curr.execute(
            "SELECT item_type, SUM(quantity) as quantity FROM bulk_items "
            "WHERE auction_id = ? GROUP BY item_type",
            (id,),
        ).fetchall()
    ...
    def _get_sold_data_month(self, month: int, year: int, id=None) -> dict:
        ...
        data = {"items": itemsData, "bulk": bulkData, "shipping": shippingData}
    ...
    def generatePurchaseReport(self, id: int):
        ...
        elements.append(
            Paragraph(
                f"Sales Report - {auctionData['auctionName']} - Added: {auctionData['dateCreated']}",
                styles["Heading1"],
            )
        )
