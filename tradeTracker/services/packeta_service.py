...
DEFAULT_HOME_DELIVERY_CARRIERS = {
    ...
    (
        "hu",
        "express one",
    ): 3828,  # HU Express One HD (free to 100k HUF; 220k +2.60; HARD CAP 220k HUF)
    ...
    (
        "be",
        "belgian post",
    ): 7909,  # BE Belgian Post HD (free to 500) — low tier (cheaper than Dutch+fee at 200)
...
class PacketaService:
    ...
    def create_packet(self, order, homeDelivery=False):
        ...
        categories = [
            c.strip().lower() for c in str(article_info.get("articleCategory") or "").split(",")
        ]
    ...
    def packets_labels_pdf(self, packetIds: list):
        ...
        labes = self.client.service.packetsLabelsPdf(
            self.api_key, PacketIds(id=packetIds), self.OFFSET, self.FORMAT
        )
        return labes
    ...
    def packet_courier_labels_pdf(self, packetIds: list):
        ...
        packets = PacketIdsWithCourierNumbers(
            packetIdWithCourierNumber=[
                PacketIdWithCourierNumber(packetId=p.packetId, courierNumber=p.courierNumber)
                for p in packetIds
            ]
        )
        labels = self.client.service.packetsCourierLabelsPdf(
            self.api_key, packets, self.OFFSET, self.FORMAT
        )
        return labels
